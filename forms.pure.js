function Field (elementId)
{
    ...
    {	
      ...
      if (this.type === "select-one" || this.type === "select-multiple")
      {
        ...
        if (selected !== null)
        {
          for (var i = 0; i < selected.length; i++)
          {
            if (json === "")
            {
              json = json + this.name+"="+selected[i];
            }
            else
            {
              json = json + "&" + this.name+"="+selected[i];
        ...
        //If this select box has no selections made, exclude it from the JSON string entirely
        else
        {
          return ""; 
        }
      ...
      else
      {
        if (json === "")
        {
          json = json + this.name+"="+this.value;
        }
        else
        {
          json = json + "&" + this.name+"="+this.value;
...
function Form (formId)
{
   ...
   this.getAsJSON = function getAsJSON()
   {
      ...
      //Get the JSON values for each field
      for (var i = 0; i < fields.length; i++)
      {
      	var value = fields[i].getAsJSON();
      	
        if (json === "")
        {
          if (value !== "") //Only add value to the JSON string
          {
            json = json + value;
          }
        }
   ...
   this.submit = function submit()
   {
     ...
     {
       return "Forms must have a valid method and action prior to non-AJAX submission";
     }
